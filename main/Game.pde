class Game {
  ...
  Bomb explodingBomb = null;
  
  // --- 遊戲結束系統 ---
  boolean gameOver = false;
  int winner = 0; // 0 = 無, 1 = 玩家1, 2 = 玩家2
  // ------------------
  ...
  void display() {
    ...
    text("Player 1 (Blue): W=Jump, A=Left, D=Right, V=Bomb", 10, 20);
    text("Player 2 (Orange): UP=Jump, LEFT, RIGHT, M=Bomb", 10, 40);
  ...
  void displayGameOver() {
    ...
    text("Press C to Return to Menu", width / 2, height / 2 + 100);
  }
  ...
  void throwBomb(Player player) {
    ...
    Bomb newBomb = new Bomb(bombX, bombY, bombVx, bombVy);
    bombs.add(newBomb);
  }
}
